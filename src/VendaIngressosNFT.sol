...
contract VendaIngressosNFT is ERC721URIStorage, Ownable {
    ...
    struct Evento {
        ...
        mapping(address => uint256) revendedores; // autorizados com quantidade
    }
    ...
    function comprarIngresso(uint256 _idEvento, string memory tokenURI) external payable {
        Evento storage evento = eventos[_idEvento];
    ...
    function venderComoRevendedor(uint256 _idEvento, string memory tokenURI) external {
        Evento storage evento = eventos[_idEvento];
